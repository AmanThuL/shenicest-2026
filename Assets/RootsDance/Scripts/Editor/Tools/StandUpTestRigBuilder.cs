...
{
    /// <summary>
    /// Adds the stand-up clip to the arms rig already wired into PlayerTest_Gameplay by
    /// <see cref="HelmetTestRigBuilder"/>, so it can be judged in Play mode (hotkey S via
    ...
    /// Menu: RootsDance > Build Stand Up Test Rig.
    /// </summary>
    public static class StandUpTestRigBuilder
    {
        ...
        private const string k_Fbx = "Assets/RootsDance/Meshes/Characters/Arms_StandUp.fbx";
        ...
        private const string k_StateName = "StandUp";
        private const Key k_Key = Key.S;

        [MenuItem("RootsDance/Build Stand Up Test Rig")]
        public static void Build()
        {
            ...
            if (clip == null)
            {
                Debug.LogError($"StandUpTestRigBuilder: no clip in {k_Fbx}. Export it from Blender first.");
                return;
            }
            ...
            if (arms == null)
            {
                Debug.LogError("StandUpTestRigBuilder: no 'Arms' object in the level. "
            ...
            {
                Debug.LogError("StandUpTestRigBuilder: the Arms object has no Animator.");
                return;
            }
            ...
            Debug.Log($"StandUpTestRigBuilder: {k_StateName} wired. Press Play and hit {k_Key}. "
        ...
        private static void AddState(AnimationClip clip)
        {
            ...
            if (controller == null)
            {
                Debug.LogError($"StandUpTestRigBuilder: {k_Controller} not found.");
                return;
            }
