...
- **Licence: CC BY 4.0** — https://creativecommons.org/licenses/by/4.0/. Work: *Siii Lab -
  Abstracted Cloth Landscape* by siii-lab (https://sketchfab.com/siii-lab),
  https://sketchfab.com/3d-models/siii-lab-abstracted-cloth-landscape-6f202b59d78d4c278585b0da52c346fc.
  The download ships no licence file; the attribution is the one recorded in
  `SourceArt/Corridor/README.md` by whoever brought the model in.
...
- **Local edit:** in `SourceArt/Corridor/RootsDance_Corridor_Blockout.blend` the chapel is taken
  apart into its 21 pieces and re-laid as an indoor corridor, and its materials are simplified to
  three. The meshes and their UVs are untouched, which is why the level builder can put the
  authored bakes back on the re-laid pieces.
- **Derived assets:** `Assets/RootsDance/Meshes/Environment/ChapterHouse/ChapterHouseCorridor.fbx`
  (exported from that blend through `Tools/blender/export_fbx.py`; manifest
  `SourceArt/Export/ChapterHouseCorridor.export.json`) and
...
- **Also present, used by nothing:** `Assets/RootsDance/Meshes/Environment/ChapterHouse/
  ChapterHouse.obj` and its `.mtl` — the untouched chapel, imported before the blockout was the
  level's source. Safe to remove once nobody wants the raw mesh in-project.

## Metal bridge — art asset, `SourceArt/Corridor/RootsDance_Corridor_Blockout.blend`

- **Licence: CC BY 4.0** — https://creativecommons.org/licenses/by/4.0/. Work: *Bridge_ Metal_10_MB*
  by Mehdi Shahsavan (https://sketchfab.com/ahmagh2e),
  https://sketchfab.com/3d-models/bridge--metal-10-mb-5933ffc71ca5427da825715660fa16d8.
- **What is used:** the long bridge component only.
- **Local edit:** everything but that component is dropped; it is rotated, scaled and positioned to
  cross the chapter house hall.
- **Derived assets:** the `Bridge_Metal_Center.001` piece of `ChapterHouseCorridor.fbx`.

## Plant monster (小花 / the flower sprite) — art asset, `SourceArt/Blender/plant-monster/`

- **Licence: UNVERIFIED — resolve before the asset ships in a build.** The download carries no
  licence file and no source URL: the folder holds only `source/plant monster.fbx` and two
  textures, in the layout a Sketchfab download unpacks to. Whoever fetched it should add the model
  URL, the author and the licence terms here, the way the Garage entry above does.
- **What is used:** `source/plant monster.fbx` — one skinned mesh (`low res`, 2,224 polygons, 42
  vertex groups) on a Rigify metarig, with four takes: `idle`, `walk`, `attack`, `agrro` — plus
  `textures/plant color.png` and `textures/plant normal.png`.
- **No local edits** to the source files.
- **Derived assets:** `Assets/RootsDance/Meshes/Characters/FlowerSprite.fbx` (the same FBX),
  `Assets/RootsDance/Textures/Characters/FlowerSprite/`, and the material, animator and prefab
  generated by `RootsDance > Content > Build Flower Sprite`. The mesh is authored 9.09 m tall and
  imports at 0.1155 so she stands 1.11 m — a little over half the player's 1.8 m — which is the
  size the script's 小花 is written at.
