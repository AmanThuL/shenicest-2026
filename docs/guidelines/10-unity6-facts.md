...
> **Applies to:** every agent and human writing C#, shaders, package manifests or build settings for this project. Usage rules live in [04](./04-unity-scripting-rules.md), [07](./07-rendering-hdrp.md) and [09](./09-packages-systems.md); this document only states facts and diffs.
...
9. **MUST** write custom rendering as HDRP **Custom Passes** / **Custom Post Process**; `ScriptableRendererFeature`, `RecordRenderGraph` and URP Compatibility Mode do not exist in this project (rule: [07](./07-rendering-hdrp.md#4-custom-rendering-custom-passes-and-custom-post-process)).
...
- **History:** the project was created on 2026-08-24 from the Hub's **Universal 3D** template ("an empty project for 3D applications. URP is pre-configured with 3D renderer"; Hub path **Projects → New project → Universal 3D → Create project**), then **migrated to HDRP 17.3.0 on 2026-08-27**. It now behaves like a **High Definition 3D** template project; the Universal 3D origin only explains leftovers, never current conventions.
  *Why:* an agent that reads "created from Universal 3D" and stops there writes URP shaders and renderer features into an HDRP project.
  *Source:* [manual-creating-a-new-project-with-urp](../reference/rendering-urp/manual-creating-a-new-project-with-urp.md) (the template we started from); layout of what the template left behind, what we deleted and the `Settings/HDRP/` layout that replaced it is in [02 Project structure](./02-project-structure.md).
- **HDRP is not compatible with URP or the Built-in Render Pipeline.** Materials, shaders (`Universal Render Pipeline/Lit`, `Standard`), `UniversalAdditionalCameraData`/`UniversalAdditionalLightData` components, renderer features and lighting advice written for those pipelines do not work here — they render magenta or are silently dropped. HDRP replacements: `HDRP/Lit`, `HDRP/Unlit`, `HDRP/TerrainLit`, `HDAdditionalCameraData`, `HDAdditionalLightData`, Custom Passes.
  *Why:* most Unity material/shader/lighting snippets on the internet are Built-in or URP; copying one in is the single most common way to break this project's rendering.
  *Source:* [HDRP package manual (index)](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-index.md) ("Projects that you make with HDRP aren't compatible with the Universal Render Pipeline"), [Choose a render pipeline](../reference/rendering-urp/manual-choose-a-render-pipeline.md)
- HDRP 17.3 ships as a **core package** (`com.unity.render-pipelines.high-definition`, `"source": "builtin"` in the lock file, fixed to the Editor version and not changeable in Package Manager) — exactly as URP 17.3 did. `com.unity.render-pipelines.high-definition-config` comes along as a transitive dependency and is never embedded.
  *Why:* nobody can "upgrade HDRP" independently of the Editor; a page for HDRP 14/16 describes a different pipeline version than the one installed.
  *Source:* [manual-pack-core](../reference/packages/manual-pack-core.md), [Install HDRP](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-install-hdrp.md), [What's new in HDRP 17](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-whats-new-17.md)
- HDRP does **not** support the Web platform (WebGL/WebGPU) or mobile: this project builds desktop standalone only. Do not install Web Build Support "just in case" and do not add a Web build profile.
  *Why:* a Web build target in an HDRP project produces a player that cannot render.
  *Source:* [HDRP system requirements](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-system-requirements.md), [manual-system-requirements](../reference/unity6-release/manual-system-requirements.md)
...
- The Hub is the recommended way to install Editors and open projects; Windows-on-Arm Editors need Hub 3.7.0 Beta 1+, Editor deeplinks need Hub 3.15+. Installing the same **desktop** modules (Windows/Mac Build Support, IL2CPP, …) on every teammate's machine avoids "wrong Editor version" prompts — no Web Build Support, per the HDRP platform note above.
...
### Rendering (facts only — conventions are in [07 Rendering](./07-rendering-hdrp.md))

Rows marked **(URP — not used here)** are true Unity 6 facts about the pipeline this project no longer uses; they stay because agents keep meeting URP snippets and need to recognise them. Anything you actually write goes through [07](./07-rendering-hdrp.md).

| Change | Introduced | Source |
|:-------|:-----------|:-------|
| **(URP — not used here)** **Render Graph** is the URP rendering path; custom passes must be written with the render graph API (`ScriptableRenderPass` + `AddRenderPasses`, `AddRasterRenderPass/ComputePass/UnsafePass`, imported textures). Legacy `AddRenderPass()` and the shared-texture workflow are deprecated in 6.3 | URP 17 / 6.0 | [manual-upgrade-guide-unity-6](../reference/unity6-release/manual-upgrade-guide-unity-6.md), [manual-urp-whats-new](../reference/unity6-release/manual-urp-whats-new.md), [releases-6000-3-0f1](../reference/unity6-release/releases-6000-3-0f1.md) |
| **(URP — not used here)** URP **Compatibility Mode (Render Graph disabled)** deprecated in 6.0, **removed in 6.3**: code stripped, `RenderGraphSettings.enableRenderCompatibilityMode` is read-only `false`; `URP_COMPATIBILITY_MODE` define re-adds it for conversion only and stops working in 6.4 | 6.3 | [manual-upgradeguideunity63](../reference/unity6-release/manual-upgradeguideunity63.md) |
| **(URP — not used here)** `ScriptableRendererFeature.SetupRenderPasses` deprecated; `AfterRendering` injection point now always runs after the final back-buffer blit (use `AfterRenderingPostProcessing` to keep rendering into an intermediate texture) | 6.2 | [manual-upgradeguideunity62](../reference/unity6-release/manual-upgradeguideunity62.md) |
| **(URP — not used here)** Shader keyword `_FORWARD_PLUS` → `_CLUSTER_LIGHT_LOOP`; **Deferred+** rendering path (HDRP has no Forward+ — its Lit Shader Mode is Forward Only / Deferred Only / Both, and this project uses **Forward Only**); PVRTC deprecated | 6.1 | [manual-upgradeguideunity61](../reference/unity6-release/manual-upgradeguideunity61.md), [manual-whatsnewunity61](../reference/unity6-release/manual-whatsnewunity61.md) |
...
| **HDRP** custom rendering is **Custom Passes** (a `CustomPassVolume` + `CustomPass` subclass, injected at a fixed injection point) and **Custom Post Process** (`CustomPostProcessVolumeComponent`, registered in the HDRP global settings). There is no `ScriptableRendererFeature` and no renderer asset in HDRP | HDRP 17 | [custom-passes-understand](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-passes-understand.md), [custom-post-processing-understand](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-post-processing-understand.md) |
| **HDRP** has no Forward+: the HDRP asset's **Lit Shader Mode** is Forward Only / Deferred Only / Both. The **SRP Batcher is always on** (no checkbox); **GPU Resident Drawer** and **GPU occlusion culling** live on the HDRP asset's *Rendering* section | HDRP 17 | [forward-and-deferred-rendering](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-forward-and-deferred-rendering.md), [srpbatcher](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-srpbatcher.md), [hdrp-asset](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-asset.md) |
| **HDRP** cameras carry `HDAdditionalCameraData` and lights `HDAdditionalLightData`; light intensities are in **physical units** (lux / lumen / nits) and exposure is a Volume override — there is no camera stacking | HDRP 17 | [hdrp-camera-component-reference](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-camera-component-reference.md), [physical-light-units](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-physical-light-units.md), [override-exposure](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-override-exposure.md) |
...
| **(URP — not used here)** `ScriptableRendererFeature.SetupRenderPasses`, `ScriptableRenderer.cameraColorTarget` | in **this** project: an HDRP **Custom Pass** (`CustomPassVolume` + `CustomPass`) — there is no renderer feature to port. In URP the 6.x replacement was a render-graph pass via `AddRenderPasses` with `RTHandle` targets | Deprecated / Obsolete (URP) | [custom-passes-understand](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-passes-understand.md), [manual-upgradeguideunity62](../reference/unity6-release/manual-upgradeguideunity62.md), [manual-upgrade-guide-unity-6](../reference/unity6-release/manual-upgrade-guide-unity-6.md) |
| **(URP — not used here)** `RenderPassEvent.AfterRendering` (expecting an intermediate texture) | in **this** project: an HDRP custom-pass **injection point** (`BeforePostProcess` / `AfterPostProcess` / …) on the `CustomPassVolume`. `RenderPassEvent` does not exist in HDRP | Behaviour change (6.2, URP) | [custom-pass-injection-points](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-injection-points.md), [manual-upgradeguideunity62](../reference/unity6-release/manual-upgradeguideunity62.md) |
| **(URP — not used here)** Render Graph `AddRenderPass()` + shared textures | in **this** project: nothing — you do not write render-graph passes. HDRP owns its render graph; you override `CustomPass.Execute` instead. (The URP-side replacement was `AddRasterRenderPass` / `AddComputePass` / `AddUnsafePass` + imported textures) | Deprecated (6.3, URP) | [custom-pass-scripting](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-scripting.md), [releases-6000-3-0f1](../reference/unity6-release/releases-6000-3-0f1.md) |
| **(URP — not used here)** `#pragma multi_compile _FORWARD_PLUS` | in **this** project: neither keyword — HDRP has no Forward+ light loop; its Lit Shader Mode is Forward Only (ours) / Deferred Only / Both, and HDRP shaders come from Shader Graph with an HDRP target. (`_CLUSTER_LIGHT_LOOP` replaced `_FORWARD_PLUS` inside URP) | Replaced (6.1, URP) | [forward-and-deferred-rendering](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-forward-and-deferred-rendering.md), [manual-upgradeguideunity61](../reference/unity6-release/manual-upgradeguideunity61.md) |
| **(URP — not used here)** `SHADER_QUALITY_LOW/MEDIUM/HIGH`, `SHADER_HINT_NICE_QUALITY` | `SHADER_API_MOBILE` / `SHADER_API_GLES` | Removed (URP 13) | [manual-upgrade-guide-unity-6](../reference/unity6-release/manual-upgrade-guide-unity-6.md) |
| Post Processing Stack v2 (`com.unity.postprocessing`) | the **Volume** framework — in this project HDRP Volume overrides and Volume profiles ([07 §6](./07-rendering-hdrp.md#6-post-processing-via-volumes)); PPv2 is unsupported under both SRPs | Unsupported in URP and HDRP | [post-processing-main](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-post-processing-main.md), [manual-integration-with-post-processing](../reference/rendering-urp/manual-integration-with-post-processing.md), [manual-upgrade-guide-unity-6](../reference/unity6-release/manual-upgrade-guide-unity-6.md) |
...
|:--------|:---|:-------------------|:------|:-------|
| High Definition Render Pipeline | `com.unity.render-pipelines.high-definition` | 17.3 | Core — **installed** (the project's pipeline since 2026-08-27) | [manual-pack-core](../reference/packages/manual-pack-core.md), [install-hdrp](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-install-hdrp.md) |
| Universal Render Pipeline | `com.unity.render-pipelines.universal` | 17.3 | Core — **removed 2026-08-27** (mutually exclusive with HDRP; never re-add, see [09](./09-packages-systems.md)) | [manual-pack-core](../reference/packages/manual-pack-core.md) |
...
| Post Processing Stack v2 (`com.unity.postprocessing` 3.5) | Listed as released, but "URP is not compatible with the Post Processing Stack v2 package" | HDRP Volume framework | [manual-integration-with-post-processing](../reference/rendering-urp/manual-integration-with-post-processing.md), [manual-upgrade-guide-unity-6](../reference/unity6-release/manual-upgrade-guide-unity-6.md) |
| **(URP — not used here)** URP Compatibility Mode | Removed (6.3) | Render Graph | [manual-upgradeguideunity63](../reference/unity6-release/manual-upgradeguideunity63.md) |
...
Relevant rows for a desktop-only team (HDRP supports neither Web nor mobile); full tables incl. mobile/console/XR are in the source.
...
| **Player — Web** (**not targeted** — HDRP does not support WebGL/WebGPU) | Desktop Chrome / Firefox / Safari / Edge that are WebGL 2.0-capable, HTML5, 64-bit, WebAssembly-capable; mobile iOS Safari 15+ / Chrome 58+ (Safari 18.2+ for the higher memory limit) |
...
- ❌ Writing a `ScriptableRendererFeature`/`ScriptableRenderPass`, enabling "Compatibility Mode (Render Graph Disabled)" or defining `URP_COMPATIBILITY_MODE` to make an old renderer feature work → ✅ none of those types exist in an HDRP project: write a **Custom Pass** or a **Custom Post Process** ([07](./07-rendering-hdrp.md#4-custom-rendering-custom-passes-and-custom-post-process)).
- ❌ Assigning a `Universal Render Pipeline/Lit` or `Standard` shader to a material, or adding `UniversalAdditionalCameraData`/`UniversalAdditionalLightData` in a scene → ✅ `HDRP/Lit` (or `HDRP/TerrainLit`, `HDRP/Unlit`) and `HDAdditionalCameraData`/`HDAdditionalLightData`; anything else renders magenta or is dropped.
...
- [ ] Custom rendering is an HDRP Custom Pass / Custom Post Process; no `ScriptableRendererFeature`, no `RecordRenderGraph`, no `SetupRenderPasses`, no `URP_COMPATIBILITY_MODE`, no `_FORWARD_PLUS`; no URP shader or `Universal*` component anywhere under `Assets/`.
...
80. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-index.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-index.md) — High Definition Render Pipeline overview (HDRP is not compatible with URP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/index.html
81. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-install-hdrp.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-install-hdrp.md) — Install HDRP — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/install-hdrp.html
82. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-whats-new-17.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-whats-new-17.md) — What's new in HDRP 17 (Unity 6) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/whats-new-17.html
83. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-upgrading-from-2023-2-to-unity-6-preview.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-upgrading-from-2023-2-to-unity-6-preview.md) — Upgrade from HDRP 16.x to 17.x — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Upgrading-from-2023.2-to-unity-6-preview.html
84. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-system-requirements.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-system-requirements.md) — HDRP system requirements (no Web, no mobile) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/System-Requirements.html
85. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-passes-understand.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-passes-understand.md) — Understand custom passes (HDRP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-passes-understand.html
86. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-post-processing-understand.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-post-processing-understand.md) — Understand custom post-processing (HDRP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-post-processing-understand.html
87. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-forward-and-deferred-rendering.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-forward-and-deferred-rendering.md) — Forward and Deferred rendering (Lit Shader Mode) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Forward-And-Deferred-Rendering.html
88. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-srpbatcher.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-srpbatcher.md) — Scriptable Render Pipeline Batcher (HDRP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/SRPBatcher.html
89. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-asset.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-asset.md) — HDRP Asset reference — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/HDRP-Asset.html
90. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-camera-component-reference.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-hdrp-camera-component-reference.md) — HDRP Camera component reference (`HDAdditionalCameraData`) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/hdrp-camera-component-reference.html
91. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-physical-light-units.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-physical-light-units.md) — Physical Light Units and intensities — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Physical-Light-Units.html
92. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-override-exposure.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-override-exposure.md) — Set the exposure (Exposure Volume override) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Override-Exposure.html
93. [../reference/rendering-urp/manual-choose-a-render-pipeline.md](../reference/rendering-urp/manual-choose-a-render-pipeline.md) — Choose a render pipeline (6000.3 Manual) — https://docs.unity3d.com/6000.3/Documentation/Manual/choose-a-render-pipeline.html
94. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-injection-points.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-injection-points.md) — Custom pass injection points (HDRP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Custom-Pass-Injection-Points.html
95. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-scripting.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-custom-pass-scripting.md) — Write a custom pass in C# (HDRP) — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Custom-Pass-Scripting.html
96. [../reference/rendering-hdrp/render-pipelines-high-definition-17-3-post-processing-main.md](../reference/rendering-hdrp/render-pipelines-high-definition-17-3-post-processing-main.md) — Post-processing in HDRP — https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Post-Processing-Main.html
