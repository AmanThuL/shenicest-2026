...
7. **MUST** use `UnityEngine.Awaitable` for async work (not `Task`, not UniTask) and pass `destroyCancellationToken` / `Application.exitCancellationToken` (rule: [04](./04-unity-scripting-rules.md)).
...
namespace RootsDance.Player
{
    public class JumpPad : MonoBehaviour
    {
        ...
        // ✅ Unity 6: Awaitable instead of Task/UniTask; token stops it on destroy.
...
| `Task`-returning async methods, UniTask | `async Awaitable` / `Awaitable<T>`; `AwaitableCompletionSource` instead of `TaskCompletionSource` | Project rule | [manual-async-awaitable-introduction](../reference/scripting/manual-async-awaitable-introduction.md) **[project decision]** |
...
- ❌ `async Task` methods, `Task.Delay`, UniTask, `.Result` on the main thread → ✅ `async Awaitable` + `Awaitable.WaitForSecondsAsync(t, destroyCancellationToken)`.
...
- [ ] Async code returns `Awaitable` / `Awaitable<T>` and takes a `CancellationToken`; no UniTask/`Task.Run` wrappers.
