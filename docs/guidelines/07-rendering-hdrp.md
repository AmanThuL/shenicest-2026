...
- *Why:* A weight-blended overlay profile keeps the level profile and the opening volumes as the single authored source: `NightProfile` overrides only **Exposure (Fixed, EV100 5)**, **Gradient Sky** colours/exposure and the Fog's **Albedo / Anisotropy / Multiple Scattering** — not the fog density, height or quality — so the 8 → 22 → 40 m opening fog ramp reads identically at night. It is an *overlay*, not a level profile, so the §5.9 Visual Environment and §5.6 Shadows requirements do not apply to it. Sun intensity blends in log space (`TimeOfDayBlend.LerpLux`) because 12 000 → 8 lux spans three decades.
...
**SHOULD** give the player's flashlight exactly this shape: a **Spot** on the Player prefab's `Head/Flashlight` at zero offset (so it is the screen centre the first-person camera hard-locks to), **2 000 lumen**, 45° cone (inner ≈ 25°), range 30 m, colour temperature 5 000 K, shadows on at the Medium tier, **Volumetrics on, Multiplier 1, Shadow Dimmer 0**. It is one of the four volumetric lights in the §11 budget. `FlashlightController` switches it on when the phase becomes Night and toggles it on `Player/Flashlight` (F / D-pad up); installed by `RootsDance > Player > Install Flashlight`. **[project decision]**
- *Why:* The cone in the volumetric fog is the night look; per-light volumetric shadows would sample the shadow map per froxel for no visible gain at this range (§5.8).
...
**MAY** add a local **Box Volume** (collider **Is Trigger**, higher **Priority**) for an area look. Set the camera's **Volume Anchor Override** to the player Transform if a future camera ever sits far from the player, so the player's position selects the volume. The 00章前段 atmosphere (`docs/design/00章前段环境设计_起始点至异色草带.md`) is the first user of this pattern: four local Box Volumes `OpeningVolume_{Wake,Ridge,Camp,Threshold}` sit under `_Lighting/OpeningAtmosphere` in `Main_Environment`, each pointing at its own `Opening{Wake,Ridge,Camp,Threshold}Profile` in `Settings/VolumeProfiles/`, built by **RootsDance > Environment > Build Opening Atmosphere** — the menu seeds a profile's look values only when it creates that profile; **RootsDance > Environment > Rebuild Opening Atmosphere Profiles (overwrite)** is the explicit entry that resets already-tuned profiles.
