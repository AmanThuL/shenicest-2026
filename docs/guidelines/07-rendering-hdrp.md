...
Current scenes: `Sun` = **20 000 lux** in both `Main` and `PlayerTest`, colour temperature off, shadows on, Volumetrics **Multiplier = 1**; the level profiles use **Fixed EV100 12.5**; Gradient Sky **Intensity Mode = Exposure, Exposure = 12**. These were tuned by screenshot so mid-tones match the pre-migration baseline (nothing clipped, nothing black). **[project decision]**
...
**MUST** enable fog through the level profile's **Fog** override (**State = Enabled**) with **Volumetric Fog** on. Current values: **Fog Attenuation Distance = 400 m** (the mean free path — at that distance the fog has absorbed and out-scattered 63 % of the background), **Base Height = 0**, **Maximum Height = 60**, neutral **Tint (0.78, 0.80, 0.84)**, everything else default. **[project decision]**
...
**MAY** add a local **Box Volume** (collider **Is Trigger**, higher **Priority**) for an area look. Set the camera's **Volume Anchor Override** to the player Transform if a future camera ever sits far from the player, so the player's position selects the volume.
