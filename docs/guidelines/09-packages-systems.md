...
| Unity Pipeline `com.unity.pipeline` | 0.5.0-exp.1 (experimental) | **Proposed** — Editor/agent tooling only (official Unity CLI `unity command …` against the open Editor); on branch `chore/agent-debug-workflow`, team decision pending; trade-offs and rules in [Unity CLI agent workflow](../architecture/tooling/unity-cli-agent-workflow.md#3-the-package-decision-comunitypipeline-050-exp1) | [live](https://docs.unity3d.com/Packages/com.unity.pipeline@0.5/manual/index.html) |
...
Removed from the Universal 3D template manifest at import (2026-08-24), all with zero dependents in the lock file: `com.unity.collab-proxy` (Unity Version Control plugin — this project is Git-only, [06](./06-version-control.md) rule 15), `com.unity.multiplayer.center` (single-player game), `com.unity.pipeline` 0.5.0-exp.1 (experimental — forbidden below; re-proposed 2026-08-25 for Editor/agent tooling, see its table row), `com.unity.timeline` and `com.unity.visualscripting` (per this table). `com.unity.cinemachine` 3.1.7 was added at the same time. **[project decision]**
