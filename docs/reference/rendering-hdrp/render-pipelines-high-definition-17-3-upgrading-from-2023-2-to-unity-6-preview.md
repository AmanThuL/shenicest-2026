---
title: "Upgrade from HDRP 16.x to 17.x"
page_title: "Upgrading HDRP from 2023.2 to Unity 6 Preview | High Definition Render Pipeline | 17.3.0"
source_url: "https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Upgrading-from-2023.2-to-unity-6-preview.html"
final_url: "https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Upgrading-from-2023.2-to-unity-6-preview.html"
...
The [Raytracing Quality Keyword](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/SGNode-Raytracing-Quality.html) has been updated to include a Pathtraced input. In previous versions, when Path tracing is enabled, the default input was used. Now, it uses the Pathtraced input. This is to prevent compilation error in graphs using unsupported nodes. If you had Shader Graph materials using the Raytracing Quality Keyword, the result will stay unchanged until you re-save them. To upgrade the behavior, you need to delete the keyword in the blackboard and re-add it manually.
...
The pixel shader variant of the deferred lighting pass was removed, the lighting is now always computed with a compute shader. The framesetting to enable this option was deleted.
...
To disable `PrecomputedAtmosphericAttenuation`, first you need to install the HDRP config package which can be done using the [Render Pipeline Wizard](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Render-Pipeline-Wizard.html). For more info, see [HDRP Config](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/configure-a-project-using-the-hdrp-config-package.html). Once installed, go in ShaderConfig.cs and set `PrecomputedAtmosphericAttenuation` to 0.
