---
title: "Create a custom pass GameObject"
page_title: "Create a Custom Pass GameObject | High Definition Render Pipeline | 17.3.0"
source_url: "https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html"
final_url: "https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html"
...
<span id="Full-Screen-Custom-Pass"></span>
...
To create a full-screen Custom Pass, click the Add (**+**) button at the bottom right of the [**Custom Pass Volume**](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Custom-Pass-Volume-Workflow.html) component and select **FullScreeenCustomPass**.

For information about the Full-screen Custom Pass properties, refer to [Custom pass reference](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-reference.html).
...
- [Create a Material from a full-screen shader graph](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#material-from-fullscreen-shadergraph).
- [Create a Material from a full-screen Custom Pass shader](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#material-from-fullscreen-custompass).

You can also create the material and its shader directly from the **FullScreen Material** property in the [Custom Pass Volume](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-reference.html) Inspector by using the **New** dropdown with the following options:
...
<span id="material-from-fullscreen-shadergraph"></span>

### Create a Material from a full-screen Shader graph

HDRP includes the [Fullscreen Master stack](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/fullscreen-master-stack-reference.html) to create a full-screen shader graph that you can use with a custom pass.

To use a Fullscreen shader graph in a custom pass:

1.  Create a Fullscreen shader graph (Menu: **Assets** \> **Create** \> **Shader Graph** \> **HDRP** \> **Fullscreen Shader Graph**) or use one of the [Fullscreen Samples](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/create-a-fullscreen-material.html#fullscreen-samples).
...
3.  Create a [Fullscreen Custom Pass](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#full-screen-custom-pass) and open it in the inspector.
...
**Important:** If you use a C# custom pass script that uses the `Blit()` function, you need to take extra steps to make the full screen shader work. For more information, see [Make a Fullscreen material Blit compatible](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/create-a-fullscreen-material.html#fullscreen-blit-compatible).

<span id="material-from-fullscreen-custompass"></span>
...
2.  Go to the **Hierarchy** and right click on the **Custom FullScreen Pass** shader, then select **Create \> Material.** Unity creates a new material with the prefix “FullScreen\_”.
3.  In the Custom Pass component, select **Add** (**+**) to create a Custom Pass and select **FullScreenCustomPass.**
4.  To assign this material to your **FullScreenCustomPass,** open the material picker using the icon at the right of the **FullScreen Material** box and select your “FullScreen\_...” material.
...
<span id="Draw-Renderers-Custom-Pass"></span>

<span id="readwrite-color-buffer"></span>
...
- [A custom pass that writes the Camera’s color buffer into a temporary (custom) color buffer.](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#write-custom-buffer)
- [A custom pass that reads from the temporary color buffer.](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#read-custom-buffer)
...
<span id="write-custom-buffer"></span>

### Write to a custom color buffer

To write to the custom color buffer in a full-screen custom pass:

- Create a [full-screen Custom Pass](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#Full-Screen-Custom-Pass).
...
<span id="read-custom-buffer"></span>

### Read from the custom color buffer

To create and assign a full-screen shader graph that reads from the custom color buffer:

1.  [Create a new full-screen shader graph](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/create-a-fullscreen-material.html#create-fullscreen-shadergraph).
...
![Shader graph screenshot: Read from the custom color buffer.](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/images/FS_CustomColorBuffer.png)

For more information on how to modify a full-screen Shader Graph, see the [full-screen master stack](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/fullscreen-master-stack-reference.html).
...
To create a draw renderers Custom Pass, click the Add (**+**) button in the [**Custom Pass Volume**](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Custom-Pass-Volume-Workflow.html) component and select **DrawRenderersCustomPass**.

For information about the Draw renderers Custom Pass properties, refer to [Custom pass reference](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-reference.html).
...
To fix this, navigate to your HDRP Asset in your **Assets** folder (if you are using the HDRP template, this is in **Assets \> Settings**) and change the **Lit Shader Mode** to **Both**. For more information, see [Changing the depth of a renderer in your scene](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#Changing-Renderer-Depth).

<span id="Material-Injection-Point-Compatibility"></span>

## Material and injection point compatibility

[Injection Points](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Custom-Pass-Injection-Points.html) determine when Unity executes a Custom Pass Volume in the render loop.
...
<span id="Object-ID-Custom-Pass"></span>
...
<span id="Custom-Renderers-Pass-shader"></span>
...
1.  A `FragInputs` structure that contains all the geometry inputs passed into the fragment shader. For information about the properties in this structure, see [FragInput](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#Fraginputs).
2.  A `float3` that contains the current pixel's view direction.
3.  A `PositionInputs` structure that contains position-based utility properties you might require in your shader. For information about the properties in this structure, see [PositionInputs](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#PositionInputs).
...
1.  A `SurfaceData` structure that contains the final color. For information about the properties in this structure, see [SurfaceData](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#SurfaceData).
2.  A `BuiltinData` structure that contains additional information that HDRP can use to calculate particular effects. For information about the properties in this structure, see [BuiltinData](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#BuiltinData).
...
1.  An AttributesMesh structure that contains the current vertex and its properties. For information about the properties in this structure, and which defines they map to, see [AttributesMesh](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/custom-pass-create-gameobject.html#AttributesMesh).
...
For an example on how to use this method to write vertex shader code, see the following code sample. This example modifies the vertex data to slightly enlarge the mesh and avoid [z-fighting](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Glossary.html#ZFighting):
...
- In [Frame Settings](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/Frame-Settings.html): Go to **Edit** \> **Project Settings** \> **Graphics** \> **Pipeline Specific Settings** \> **HDRP**. You can then disable custom passes in one of the following places:
...
- [HDRP Asset](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.3/manual/HDRP-Asset.html): Open the HDRP Asset, navigate to **Rendering** and disable the **Custom Pass** property. This automatically disables the custom buffer allocation.
