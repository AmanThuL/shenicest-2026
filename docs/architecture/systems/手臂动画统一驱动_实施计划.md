...
**Blender 侧** —— `arms_rig_all.blend` 新增两条动作，均由既有已验证帧逐值派生，所以接缝天然精确：

| 新动作 | 帧 | 来源 | 用途 |
|---|---|---|---|
| `neutral` | 1–2，循环 | 左臂 ← `scanner_lower` f17；右臂 ← `drop` f28；root/camera 归位到站立基准 | 唯一的 neutral：双前臂垂下 |
| `hold` | 1–2，循环，仅右臂 | ← `drop` f1（≡ `grab_ground` f75） | 取物与丢弃之间的持物保持，右前臂抬起 |

逐通道核对，五个接缝全部 **EXACT**：`hold` ≡ `drop` f1、`hold` ≡ `grab_ground` f75（右臂）、
`neutral`(右) ≡ `drop` f28、`neutral`(左) ≡ `scanner_lower` f17、`neutral`(左) ≡ `scanner_raise` f1。
文件的存储姿势也按制作规约归位到了 `hang_low`（此前右臂停在 `forearm_raised`）。
`validate_wrist.py`：28 个 action 全清。

**导出** —— 新增 `Arms_Neutral` / `Arms_Hold` / `Arms_GrabGround` / `Arms_Drop` 四个 FBX，
并在 `Tools/unity/model_import_profiles.json` 注册；Unity 导入后 clip 名与 loop 标志均正确
（`Arms_Neutral` 0.033 s loop、`Arms_Hold` 0.033 s loop、`Arms_GrabGround` 2.467 s、`Arms_Drop` 0.900 s）。

**Unity 侧** —— 三层状态机由 `RootsDance > Arms > Build Arms Controller` 生成，实测结构：

```
Base      weight 1  mask 无        default Neutral   Neutral GrabGround KeypadPoke Crawl StandUp HelmetOff
LeftArm   weight 0  mask ArmLeft   default Empty     ScannerRaise ScannerLower
RightArm  weight 0  mask ArmRight  default Empty     Hold Drop
parameters: Speed0 Speed1 Speed2（每个 state 的 speed multiplier）
```

两个遮罩各 25/57 个 transform 启用，`root`、`camera`、`helmet_socket` 均**不在**任一遮罩内——
单臂 clip 因此动不了视角，符合契约。

`ArmsDirector` 是 Animator 的唯一持有者，`Animator.speed` 永远为 1；neutral 与 hold 是真实的循环
state，不是冻结帧。姿势闸门与高度基准有 16 个 EditMode 测试覆盖，全部通过
（`RootsDance.Tests.EditMode.Player`）。

**手持物** —— `HandSocket` 只跟位置和旋转、不写缩放，`CarriedItem` 承担成品尺寸与握持偏移，
§1.5 的百倍缩放在代码层面已消除。

### 仍待做

1. **接线**：`PlayerTest_Gameplay.unity` 里把 sockets 移出 `Arms.fbx` 实例层级、挂 `ArmsDirector` /
   `ArmsHeightRig` / `ArmsDebugConsole`，删除三个旧 debug trigger（涉及共享场景，改前按
   [guideline 11](../../guidelines/11-scenes-prefabs-workflow.md) 在团队频道说一声）。
2. **scanner 成品尺寸**：把 0.670016 应用进 `Scanner.blend` 并重导，`Scanner.prefab` 即 1:1。
3. **`stand_up` root 抬升**（§1.3）：压平后 `ArmsHeightRig` 才能独占高度。
4. **适配器**：`ScannerAnimatorView` / `HelmetAnimatorView` 改为 `IArmsDirector` 的薄适配器；
   `HelmetAnimatorView` 在 f27 的 Attach 事件上隐藏 rig 自带的头盔并把真头盔交给右手挂点。
5. **两条接缝靠 cross-fade 而非逐值相等**（`grab_ground` 起手 0.54 m、`helmet_off` 末帧 0.33 m）——
   见契约"待定项"，由技术美术决定是否返工。
