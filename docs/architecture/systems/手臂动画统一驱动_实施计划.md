...
- `grab_ground_tube`（1–75，双手 + camera + root）——`grab_ground` 的握试管版本，2026-08-30 新增
- `drop`（1–28，右臂）
- `throw`（1–40，右臂）——过肩投，2026-08-30 新增
- `hold_tube`（1–2，右臂，循环）——右手持试管的保持姿势，2026-08-30 新增
...
| 契约当前写法 | 实测 | 结论 |
|---|---|---|
| ~~`grab_ground` 起始假设 `hang_low`~~ | **已修正（2026-08-30 复测）**：首帧 = `hang_low`（−0.220, 0.000, 1.050），末帧 = `forearm_raised`（−0.307, −0.246, 1.524） | 这条漂移已不存在。`4b139086 soften grab_ground's wind-up into the crouch` 之后 clip 就改成从 `hang_low` 起手了，当时契约没跟上；契约已按复测值更新，`neutral → grab_ground → hold` 是逐值衔接的 |
