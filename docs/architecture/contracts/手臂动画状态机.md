...
- **无** camera / root 曲线:`scanner_raise`、`scanner_lower`、`drop`、`hold`、`throw`。
  这四条是单臂 clip,**故意**不绑 camera —— 它们在遮罩层上播,带了 camera 曲线会在举扫描仪时把视角一起拽走。
- **有** camera 曲线:`neutral`、`grab_ground`、`keypad_poke`、`crawl`、`stand_up`、`helmet_off`。
...
| 分类 | clip | Unity 侧挂点高度 |
|---|---|---|
| 站立(standing) | `neutral`、`hold`、`grab_ground`、`keypad_poke`、`scanner_raise`、`scanner_lower`、`drop`、`throw` | 站立基准(clip 内的蹲下由动画自身表达) |
