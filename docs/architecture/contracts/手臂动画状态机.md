...
| `hold_tube` | 1–2 | 右 | `forearm_raised`,右手握试管(= `grab_ground_tube` 末帧右臂,逐值相等) | 同起始;循环。右手持试管的保持姿势,唯一去向是 scanner 流程 |
...
下面这些是**作画与调用约定,不是代码闸门**:`ArmsDirector` 不因为手臂当前姿势拒绝任何动作,
请求什么就播什么(只有"同一层上一条非循环动作还没播完"仍然会被拒)。接缝对不上时看到的是一次
cross-fade 跳变,而不是一条被吞掉的动作。

- `scanner_raise` 之后应接 `scanner_lower` 回到 neutral。
- `drop` 的首帧对齐 `grab_ground` 的结束状态(起始帧逐值相等),从别处进入会有跳变。
...
2. 右臂遮罩层进入 `hold_tube` 循环(2 帧,逐值等于 `grab_ground_tube` 的 f75),把持试管的姿势保持住;
...
- **无** camera / root 曲线:`scanner_raise`、`scanner_lower`、`drop`、`hold`、`hold_tube`、`throw`。
  这六条是单臂 clip,**故意**不绑 camera —— 它们在遮罩层上播,带了 camera 曲线会在举扫描仪时把视角一起拽走。
...
| 分类 | clip | Unity 侧挂点高度 |
|---|---|---|
| 站立(standing) | `neutral`、`hold`、`hold_tube`、`grab_ground`、`grab_ground_tube`、`keypad_poke`、`scanner_raise`、`scanner_lower`、`drop`、`throw` | 站立基准(clip 内的蹲下由动画自身表达) |
...
- **单臂 clip 的腕角读数取决于文件的存储姿势。** 它不 key `root`、左臂和 camera,所以播它时这些骨停在存盘姿势上,肩的位置随之而定,底下的 IK 解算跟着变。同一套逐值相同的右臂曲线,在 `grab_ground_tube` f75 上量到 twist 8.3°、单独做成 `hold_tube` 播就是 21.5° —— 两个数都对,量的不是同一件事。`validate_wrist.py` 按顺序遍历 action,也会带着上一条的残留姿势,所以逐条单独跑才可比。
...
- **`crawl` 没有入口**:没有任何 clip 从站立进入 `crawl_pose`,直接播 `crawl` 会由 cross-fade 硬接。补一条"趴下"clip 后接缝才干净。
