...
stateDiagram-v2
    ...
    HL --> FRI : grab_ground_tube (双手, 手里已有试管) → ⚠ 缺 hold_tube
...
- `grab_ground_tube` 与 `grab_ground` 同起同止,二选一:手空着走 `grab_ground`,手里已有试管走 `grab_ground_tube`。**末帧右手姿势与 `hold` 不同**(握着试管),所以它接不上 `hold`,见「待定项」。
...
- **缺 `hold_tube`**:`hold` 的定义是"逐值等于 `grab_ground` 末帧右臂",而 `grab_ground_tube` 末帧右手是握着试管的手型,接不上 `hold`。需要补一条右臂遮罩层的 2 帧循环,逐值等于 `grab_ground_tube` 的 f75 —— 与 `hold` 之于 `grab_ground` 同样的关系。
