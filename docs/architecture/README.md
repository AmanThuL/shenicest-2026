...
| 文档 | 适用角色 | 用途 |
|---|---|---|
| [温室玻璃工作交接](温室玻璃工作交接.md) | 场景美术、技术美术 | 接手温室玻璃/门窗前先读：哪份文件是标准答案、生成链条的执行顺序、十一个反复踩中的测量陷阱（依赖图、局部坐标面积、镜像配对、UV 丢失…）、待办与验收方式。 |
| [废弃温室材质研究](废弃温室材质研究.md) | 场景美术、技术美术 | 铁/玻璃/墙三条降解链、按「材料+经历+环境」构造的检索词、已核实的 CC0 资产对照表，以及两个库都没有、必须程序化生成的层。 |
| [废墟风化分层策略](废墟风化分层策略.md) | 场景美术、技术美术 | 图案属于模块、用量属于位置：模块材质 / 世界空间叠加 / 手工贴花的三层分工，天空遮蔽烘顶点色的输入，以及玻璃破损为何是摆放问题而非贴图问题。 |
...
| [无线电与对话语音接入](systems/无线电与对话语音接入.md) | 文案、音频、gameplay 程序、技术美术 | 给一句台词配录音的完整路径：录音挂在台词行上而不是 cue 上、三条优先级决定的停留时长、无线电与对话两条链路的资产与频道、`FlagAudioBed` / `FootstepAudio` 等不写代码的接法、00 章旗标到床的对照表，以及音频文件的落盘位置与 `Voice` 混音组。 |
| [旧档案：拾取与阅读](systems/旧档案拾取与阅读.md) | gameplay 程序、美术、策划 | 拿起旧纸、举到眼前、擦灰、推近拉远、转动翻面阅读的完整流程：一个页面预制件 + 一份文档资产的拆分方式，运行时与 Briggs 场景接线已落地。 |
| [纸张折痕研究](systems/纸张折痕研究.md) | 程序（shader/合成架构）、技术美术 | 把折痕参数从"凭感觉调"变成可推导、可复核的数字；结论已定，改造清单见 §5，是[旧档案拾取与阅读](systems/旧档案拾取与阅读.md) §2 的下层研究文档。 |
...
| [Dev Play：从检查点开始玩](tooling/dev-play-checkpoints.md) | Editor 专用：一键打开检查点所属 LevelSO 的场景、进 Play、把玩家传送到站点并通过命令队列补齐世界状态（flag / 报告条目）；Play 中可跳到其他检查点；检查点是 `Data/DevPlay/` 下的 `DevCheckpointSO`（英文）。 |
| [导出版 Checkpoint 救援窗口](tooling/build-checkpoint-rescue.md) | `Ctrl + Shift + D` 打开隐藏 uGUI 面板；确认后重置运行进度并重载 checkpoint，含导出、构建校验和发布开关（英文）。 |
