...
- [x] `C-02` 第四轮复审将 C 区非实体植物总量限制为最多 `750` 株；地被 footprint gap 约 `61%`，中层 spacing 提高到 `3.82 m`。
...
- [x] `C-07` 第四轮复审后 C 调整为 `640 + 101 = 741` 株非实体植物，其中 mid 包含 1 株扫描英雄草；另保留 `37` 个实体根石封边。非实体植物比上一版减少约 `49.9%`，并低于 `750` 株上限。扫描草移至距路线约 `0.7 m`。
...
| C 可见包络、实例数和 coverage | ground `640`、mid `101`，非实体植物合计 `741`；physical `37` 另计；61% footprint gap；所有非实体植物通过双粉色材质审计 |
| A/B/D/E 各类实例数 | A `4,360/483/101`；B `5,701/503/209`；D `6,674/754/333`；E `15,930/2,611/3,675` |
| 草高 bounds 审计 | builder 对全部 `42,112` 个实例按 role/zone 范围验证通过；英雄草 `1.25 m` 单独验证 |
