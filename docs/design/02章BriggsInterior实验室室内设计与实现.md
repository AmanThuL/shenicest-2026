...
再进入主实验室完成东侧根须实验、中央实验和西侧档案调查。最终出口将位于北墙正中心，但不属于当前实现范围。
...
- 最终出口已确认为北墙正中心的圆形门。门模型由用户制作中，当前不补模型、不做替代占位、不接 Collider、灯光、VFX 或交互。
...
`GarageShell` 负责视觉，不负责行走碰撞。当前独立碰撞壳在
...
已确认边界：最终出口是北墙正中心的圆形门，门模型仍在制作中。当前阶段忽略出口设计与实现。

- 不制作临时方门、菌丝膜或其他替代模型。
- 不新增出口 Collider、Trigger、感应点、开门动画、灯光、Reflection Probe、VFX 或 Shader。
- 不让周边 PWB props、管线、根系和落地设备侵入北墙正中心的门体预留区。
- 圆门的精确直径、中心高度、门框厚度、开启方式和流程接线，等正式模型进入 Unity 后再确定。
...
当前建议检查廊道和主实验台两个 Reflection Probe 区域。圆形出口模型进入 Unity 后，再单独决定是否需要第三个 Probe。
...
S10、S11、S12 的出口表现当前暂停。正式圆形门模型进入 Unity 前，不为这些节点制作临时感应点、菌丝膜、灯光或 Shader。
...
未来再从 S9 接到北墙正中心的圆形出口。当前布景和验收不覆盖 S10、S11、S12。
...
4. 北墙正中心保留圆门空间，但不通过临时灯光或占位网格把它做成当前导航目标。
5. S9-02 隐藏残页保持可选。
...
9. 出口流程：门型已确认为北墙正中心圆门，其感应点、菌丝膜、开启方式和交互规则全部暂停，等待正式模型与后续决定。
