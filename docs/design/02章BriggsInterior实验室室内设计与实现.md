...
- 顶部结构、墙体、悬挂植物和气氛以用户确认的 `006b2dc` 为视觉基线，但不沿用该场景里误把实验室包裹在内的巨大屋顶 Transform。
  当前 `_Geometry/BriggsCeilingAssembly` 直接复用 `GarageShell.fbx` 的 `Ceiling`、`Ceiling_Beam` 和
  `Ceiling_Beam_Broken` 原始 Mesh，并把它们确定性适配到 18 × 14 m 主室。破顶是 `Ceiling` Mesh 中真实建模的开口，
  不是植被或光柱造成的视觉错觉。`GarageSourceArt/IvyHanging`、`_Props/CeilingHoleVines` 和 PWB 下 121 个室内 props 保持不变。
- `Ceiling`、`Ceiling_Beam` 和 `Ceiling_Beam_Broken` 使用原始
  `Assets/RootsDance/Materials/Environment/Garage/GarageCeiling.mat`，不再叠加后续专用 Triplanar 材质。
  主顶世界 Bounds 为中心 `(0, 5, 0)`、尺寸 `(18.8, 0.26, 14.8)`，在东西南北墙顶各覆盖约 0.1 m 并以厚度压住墙顶，
  防止周边缝隙漏光。两根梁分别适配为 `0.34 × 0.30 × 14.6 m` 和 `0.42 × 0.34 × 14.6 m`，不再跨越或包裹房间。
...
| `RoofShaft_Main` | Spot，`(0.1, 5.08, 2.5)`，4200 lm，28° | 主破洞光柱 | 体积参与度 8，绿色无阴影艺术光柱 |
| `RoofShaft_West` | Spot，`(-5.35, 5.08, 3.75)`，2800 lm，24° | 西侧破洞光柱 | 体积参与度 5，绿色无阴影艺术光柱 |
...
- 固定在墙上的密码盘和终端进入近景时，模型的世界位置、旋转和缩放必须保持不变；由设备 prefab
  内默认关闭的 Cinemachine 相机切到面板正面，退出时再切回第一人称相机。确认按键需要调整构图时移动
  近景相机，不移动墙装模型。违反这一约束会让设备表现得像被拿到玩家面前的纸张，并破坏墙体遮挡关系。
...
互相遮挡和局部空缺，但必须通过 Renderer bounds 贴合 `Y 0.94` 的工作面，禁止悬空；复合 Prefab 必须在完成旋转和缩放后
逐个子器皿贴合，不能只吸附整组最低点。所有桌面版本无行走 Collider。

当前手工布景保留主组的 6 件桌面器皿，后组 8 件已移到东侧地面；合并贴地修正时保留各自的平面位置、旋转、缩放及已删除的部件，
桌面器皿和蓝色烧瓶底部贴合 `Y 0.942`，地面后组底部贴合 `Y 0.002`（均含 2 mm 间隙）。不把后组重新移回桌面。
...
当前实现保留 `006b2dc` 的 Fixed Exposure EV 4.5。主破顶 Spot 为 4200 lm，西侧次光为 2800 lm；两束灯保持较宽角、
无阴影艺术定向配置，并从适配后的顶面开始照入室内，加强 Briggs 本地 PSX、Bloom 和绿色体积雾下的多束轮廓。
...
主灯为 `(0.1, 5.08, 2.5)`、旋转 `(70, 180, 0)`、28 度外角、12 度内角、体积参与度 8；西侧灯为
`(-5.35, 5.08, 3.75)`、旋转 `(74, 165, 0)`、24 度外角、10 度内角、体积参与度 5。两者 Range 均为 9 m，
不投射实时阴影。位置与亮度按房间尺度屋顶重新适配，不随 PWB props 改动。
