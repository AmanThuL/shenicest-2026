...
- **Creating assets from code or shell** (scripts, asmdefs, folders): follow the tree in [02](docs/guidelines/02-project-structure.md). New files get their `.meta` when a teammate next opens the Editor — commit the `.meta` together with the file; never fabricate `.meta` files.
